    ...
    <main className="relative min-h-screen overflow-hidden bg-[#000000] px-3 py-3 text-[#EAEAEA] sm:px-4 sm:py-4 lg:px-6 lg:py-6">
      ...
      <div className="relative mx-auto max-w-4xl space-y-4 lg:space-y-6">
        <Panel className="p-8 sm:p-10 lg:p-14">
          <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[#555555]">Principles</p>
        ...
        <div className="grid gap-4 md:grid-cols-2 md:gap-6">
          {PRINCIPLES.map((p) => (
            <Panel key={p.title} className="p-6">
              <h3 className="text-lg font-medium text-[#EAEAEA]">{p.title}</h3>
              <p className="mt-3 text-sm leading-7 text-[#9A9A9A]">{p.body}</p>
            </Panel>
          ))}
        </div>
