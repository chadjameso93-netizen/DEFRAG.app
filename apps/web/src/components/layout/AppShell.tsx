...
import { LayoutDashboard, Users, GitBranch, MessageSquare, Settings } from "lucide-react"
...
function NavItem({
  ...
  return (
    <Link
      href={href}
      className="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium text-zinc-600 transition hover:bg-zinc-100 hover:text-zinc-950"
    >
      ...
      <div className="mx-auto grid min-h-screen max-w-[1600px] grid-cols-1 xl:grid-cols-[280px_1fr]">
        <aside className="border-r border-zinc-200 bg-white/90 px-6 py-8">
          ...
          <nav className="space-y-2">
            ...
            <NavItem href="/dashboard" label="Relationships" icon={<Users size={18} />} />
            <NavItem href="/dashboard" label="System map" icon={<GitBranch size={18} />} />
            <NavItem href="/dashboard" label="AI chat" icon={<MessageSquare size={18} />} />
            <NavItem href="/dashboard" label="Settings" icon={<Settings size={18} />} />
          </nav>
        ...
        <section className="px-6 py-8 md:px-8 xl:px-10">
          <div className="mb-8">
            ...
            {subtitle ? (
              <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-600">{subtitle}</p>
            ) : null}
          </div>
          {children}
        </section>
